...
<LazyStackBlitz
  src="https://stackblitz.com/github/susanfulop/io-signal-form-article/tree/simple-reactive-form?embed=1&file=src/app/app.component.ts&view=editor&ctl=1"
...
<LazyStackBlitz
  src="https://stackblitz.com/github/susanfulop/io-signal-form-article/tree/advanced-reactive-form?embed=1&file=src/app/app.component.ts&view=editor&ctl=1"
  title="Advanced Reactive Form"
/>
...
<LazyStackBlitz
  src="https://stackblitz.com/github/susanfulop/io-signal-form-article/tree/simple-signal-form?embed=1&file=src/app/app.component.ts&view=editor&ctl=1"
  title="Simple Signal Form"
/>

### Hybrid model - technically _works_, but no benefits, not recommended

The current problem with forms with signals is that they lacked a fully mature API to organize complex forms in a grouped structure with all the capabilities we expect (nested grouping, dynamic validation, complex logic, etc.). Although a "cheat" workaround exists — re-using the old `FormGroup` (from _Reactive Forms_) inside a signal-based setup — but relies on the legacy _zone.js + observable & subscription model_, which undermines many of the benefits that Signals aim to deliver. As a result, using Signals with `FormGroup` in this hybrid way often offers little to no real benefit. Therefore, it is generally not recommended to combine them this way.

<LazyStackBlitz
  src="https://stackblitz.com/github/susanfulop/io-signal-form-article/tree/hybrid-form?embed=1&file=src/app/app.component.ts&view=editor&ctl=1"
  title="Hybrid Form"
/>
...
One of the most significant additions is the **experimental** release of _Signal Forms_, which address the missing feature and hopefuly be the sucessor of _Reactive Forms_.
...
<LazyStackBlitz
  src="https://stackblitz.com/github/susanfulop/io-signal-form-article/tree/experimental-advanced-signal-form?embed=1&file=src/app/app.component.ts&view=editor&ctl=1"
  title="Experimental Signal Form"
/>
