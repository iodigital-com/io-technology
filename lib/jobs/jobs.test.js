...
describe('jobs utilities', () => {
  ...
  describe('getRelatedJobs', () => {
    ...
    it('should handle tags as array', async () => {
      ...
      const reactJobs = result.jobs.filter(
        (job) => Array.isArray(job.tags.tag) && job.tags.tag.includes('React')
      )
    ...
    it('should handle tags as string', async () => {
      ...
      const nodeJob = result.jobs.find((job) => job.tags.tag === 'Node.js')
    ...
    it('should handle empty tags gracefully', async () => {
      ...
      expect(result.jobs[0]).toBeDefined()
    })
